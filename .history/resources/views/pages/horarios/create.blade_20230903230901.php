                ...
                @foreach (['lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado', 'domingo'] as $dia)
                    ...
                    <div class="border border-gray-200 rounded dark:border-gray-700 p-4">
                        <input type="time" name="{{ $dia }}_manana_entrada"
                            id="{{ $dia }}_manana_entrada" disabled>
                        <input type="time" name="{{ $dia }}_manana_salida"
                            id="{{ $dia }}_manana_salida" disabled>
                    </div>
                    <div class="border border-gray-200 rounded dark:border-gray-700 p-4">
...
<script>
    create?empleado=1:946 Uncaught TypeError: checkbox.getAttribute is not a function
    at toggleHorarios (create?empleado=1:946:37)
    at create?empleado=1:973:13
    at NodeList.forEach (<anonymous>)
    at HTMLDocument.<anonymous> (create?empleado=1:967:20)
4create?empleado=1:948 Uncaught TypeError: Cannot read properties of null (reading 'getElementsByTagName')
    at toggleHorarios (create?empleado=1:948:36)
    at HTMLInputElement.onchange (create?empleado=1:690:63)
