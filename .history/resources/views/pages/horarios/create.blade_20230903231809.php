                ...
                @foreach (['lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado', 'domingo'] as $dia)
                    ...
                    <div class="border border-gray-200 rounded dark:border-gray-700 p-4">
                        <input type="time" name="{{ $dia }}_manana_entrada"
                            id="{{ $dia }}_manana_entrada" class="time-input" disabled>
                        <input type="time" name="{{ $dia }}_manana_salida"
                            id="{{ $dia }}_manana_salida" class="time-input" disabled>
                    </div>
                    <div class="border border-gray-200 rounded dark:border-gray-700 p-4">
                        <input type="time" name="{{ $dia }}_tarde_entrada"
                            id="{{ $dia }}_tarde_entrada" class="time-input" disabled>
                        <input type="time" name="{{ $dia }}_tarde_salida"
                            id="{{ $dia }}_tarde_salida" class="time-input" disabled>
                    </div>
                @endforeach
...
<script>
    function toggleHorarios(checkbox) {
        ...
        if (checkbox.checked) {
            ...
            horariosDiv.style.backgroundColor = '#B3B3B3'; // Cambiar el fondo de la sección
        } else {
            label.querySelector('span').textContent = 'Trabaja'; // Cambiar a "Trabaja"
            label.style.backgroundColor = '';
            horariosDiv.style.backgroundColor = ''; // Eliminar el fondo gris
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        checkboxes.forEach((checkbox) => {
            toggleHorarios(checkbox); // Llamar a la función para configurar el estado inicial
        });
    });
</script>
