        ...
        <form method="POST" action="{{ route('horarios.store') }}">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2 lg:grid-cols-4">
                @foreach (['lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado', 'domingo'] as $dia)
                    <div class="flex flex-col sm:flex-row justify-between items-center border border-gray-200 rounded dark:border-gray-700 p-4">
                        <div class="flex flex-col sm:flex-row justify-between items-center">
                            <label for="{{ $dia }}" class="font-bold">{{ ucfirst($dia) }}</label>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="{{ $dia }}_descansa"
                                    id="{{ $dia }}_descansa"
                                    onchange="toggleHorarios('{{ $dia }}_descansa', '{{ $dia }}_horarios')"
                ...
                @endforeach
                <button type="submit" class="mt-4 sm:mt-0 bg-blue-600 text-white rounded py-2 px-4">Guardar</button>
            </div>
        </form>
