                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 bg-gray-50 dark:bg-gray-800">
                                <div class="grid gap-6 mb-6">
                                    <div>
                                        <input type="text" name="justificacion" id="justificacionInput"
                                            value="" disabled placeholder="Justificación"
...
<script>
    ...
    function mostrarCampoTardanza(mostrar) {
        var campoTardanza = document.getElementById("campoTardanza");
        var minutosInput = document.getElementById("minutosInput");

        if (mostrar) {
            campoTardanza.style.display = "block";
            minutosInput.setAttribute("required", "required");
        } else {
            campoTardanza.style.display = "none";
            minutosInput.removeAttribute("required");
            minutosInput.value = "";
        }
    }

    // Agregar un evento para escuchar los cambios en los radios de estado de asistencia
    var radios = document.querySelectorAll('input[type="radio"][name="estado"]');
    radios.forEach(function(radio) {
        radio.addEventListener("change", function() {
            if (radio.value === "asistio") {
                habilitarJustificacion(true);
                mostrarCampoTardanza(false);
            } else if (radio.value === "no_asistio") {
                habilitarJustificacion(false);
                mostrarCampoTardanza(false);
            } else if (radio.value === "tardanza") {
                habilitarJustificacion(false);
                mostrarCampoTardanza(true);
            }
        });
    });

    // Agregar un evento para inicializar el estado de los campos al cargar la página
    var estadoSeleccionado = document.querySelector('input[type="radio"][name="estado"]:checked');
    if (estadoSeleccionado) {
        if (estadoSeleccionado.value === "asistio") {
            habilitarJustificacion(true);
            mostrarCampoTardanza(false);
        } else if (estadoSeleccionado.value === "no_asistio") {
            habilitarJustificacion(false);
            mostrarCampoTardanza(false);
        } else if (estadoSeleccionado.value === "tardanza") {
            habilitarJustificacion(false);
            mostrarCampoTardanza(true);
        }
    }
